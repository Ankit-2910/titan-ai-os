...
import {
  ...
  Loader2, HeartPulse, Server, School, Sparkles, Repeat, Download,
} from "lucide-react";
...
const DOMAIN_META: Record<string, { name: string; icon: React.ReactNode }> = {
  healthcare: { name: "Healthcare OS", icon: <HeartPulse size={14} /> },
  it:         { name: "IT Operations OS", icon: <Server size={14} /> },
  education:  { name: "Education OS", icon: <School size={14} /> },
  general:    { name: "General", icon: <Sparkles size={14} /> },
};

function downloadFile(payload: FilePayload) {
  const bytes = atob(payload.content_base64);
  const arr = new Uint8Array(bytes.length);
  for (let i = 0; i < bytes.length; i++) arr[i] = bytes.charCodeAt(i);
  const blob = new Blob([arr], { type: payload.mime_type });
...
export default function DashboardPage() {
  ...
  const sendMessage = useCallback(() => {
    ...
    abortRef.current = chatApi.stream({
      ...
      onFile: (payload: FilePayload) => {
        setMessages((prev) =>
          prev.map((m) => m.id === asstMsgId ? { ...m, file: payload } : m)
        );
      },
      onDone: (convId) => {
  ...
  async function logout() { await authApi.logout(); router.push("/login"); }

  const dm = DOMAIN_META[domain] || DOMAIN_META.general;

  const S = {
    root: { display:"flex" as const, height:"100vh", background:"#08080c", color:"#e8e8ef",
      fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", overflow:"hidden" as const },
    sidebar: { width:260, display:"flex" as const, flexDirection:"column" as const,
      background:"#0d0d14", borderRight:"0.5px solid rgba(255,255,255,0.07)", flexShrink:0 },
    logoRow: { display:"flex" as const, alignItems:"center", gap:10, padding:16,
      borderBottom:"0.5px solid rgba(255,255,255,0.07)" },
    logoBox: { width:34, height:34, borderRadius:10, display:"flex" as const,
      alignItems:"center", justifyContent:"center",
      background:"linear-gradient(145deg,#1a1530,#0d0a1a)",
      border:"0.5px solid rgba(167,139,250,0.3)" },
    logoText: { fontWeight:600, letterSpacing:3, fontSize:15,
      background:"linear-gradient(90deg,#e9d5ff,#a78bfa)",
      WebkitBackgroundClip:"text" as const, backgroundClip:"text" as const,
      WebkitTextFillColor:"transparent" as const },
    newBtn: { width:"100%", display:"flex" as const, alignItems:"center",
      justifyContent:"center", gap:6,
      background:"linear-gradient(90deg,#7c3aed,#6366f1)", border:"none",
      color:"#fff", fontSize:13, padding:10, borderRadius:10, cursor:"pointer", marginBottom:8 },
    sectorBtn: { width:"100%", display:"flex" as const, alignItems:"center",
      justifyContent:"center", gap:6, background:"rgba(255,255,255,0.04)",
      border:"0.5px solid rgba(255,255,255,0.1)", color:"#a78bfa",
      fontSize:12, padding:8, borderRadius:10, cursor:"pointer" },
    convList: { flex:1, overflowY:"auto" as const, padding:8 },
    emptyHint: { fontSize:12, color:"#5a5a6e", textAlign:"center" as const, marginTop:20 },
    userRow: { display:"flex" as const, alignItems:"center", gap:8, padding:12,
    ...
    header: { display:"flex" as const, alignItems:"center", gap:12, padding:"12px 24px",
      borderBottom:"0.5px solid rgba(255,255,255,0.07)", background:"#0d0d14", flexShrink:0 },
    badge: { display:"flex" as const, alignItems:"center", gap:7, fontSize:13, fontWeight:500,
      color:"#c4b5fd", background:"rgba(124,58,237,0.15)", padding:"6px 12px", borderRadius:10 },
    messages: { flex:1, overflowY:"auto" as const, padding:24,
      display:"flex" as const, flexDirection:"column" as const, gap:14 },
    emptyState: { flex:1, display:"flex" as const, flexDirection:"column" as const,
      alignItems:"center", justifyContent:"center", textAlign:"center" as const, minHeight:"60vh" },
    inputArea: { padding:"14px 24px 18px",
      borderTop:"0.5px solid rgba(255,255,255,0.07)", background:"#0d0d14", flexShrink:0 },
    inputWrap: { display:"flex" as const, alignItems:"flex-end", gap:12,
    ...
    <div style={S.root}>
      ...
      <aside style={S.sidebar}>
        <div style={S.logoRow}>
          ...
          <span style={S.logoText}>TITAN</span>
        </div>

        <div style={{ padding:"12px 12px 4px" }}>
          <button onClick={newConversation} style={S.newBtn}><Plus size={14}/> New conversation</button>
          <button onClick={() => router.push("/select")} style={S.sectorBtn}><Repeat size={13}/> Change sector</button>
        </div>

        <div style={S.convList}>
          {conversations.length === 0 && <p style={S.emptyHint}>No conversations yet.</p>}
          {conversations.map((conv) => (
            <button key={conv.id} onClick={() => selectConversation(conv)} style={{
              width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between",
              padding:"9px 12px", borderRadius:9, fontSize:13, textAlign:"left", cursor:"pointer",
              ...
              <span onClick={(e) => deleteConversation(conv.id, e)}
                style={{ marginLeft:8, color:"#6b6b7e", display:"flex", alignItems:"center" }}>
                <Trash2 size={12}/>
              </span>
            </button>
          ))}
        ...
        <div style={S.userRow}>
          <div style={S.avatar}>{userEmail.charAt(0).toUpperCase()}</div>
          ...
          <button onClick={logout} style={{ background:"none", border:"none",
            color:"#6b6b7e", cursor:"pointer", display:"flex" }}>
            <LogOut size={13}/>
          </button>
        </div>
      </aside>
      ...
      <main style={S.main}>
        <header style={S.header}>
          <span style={S.badge}>{dm.icon} {dm.name}</span>
          ...
          {messages.map((msg) => (
            <div key={msg.id} style={{ display:"flex", gap:10,
              ...
              <div style={{ maxWidth:680 }}>
                  ...
                  {msg.role === "assistant" ? (
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        pre: ({children}) => <pre style={{background:"rgba(0,0,0,0.3)",padding:12,borderRadius:10,overflowX:"auto",margin:"8px 0"}}>{children}</pre>,
                        blockquote: ({children}) => <blockquote style={{borderLeft:"3px solid #7c3aed",paddingLeft:12,margin:"8px 0",color:"#b8b8c8"}}>{children}</blockquote>,
                ...
                {msg.file && (
                  <button onClick={() => downloadFile(msg.file!)}
                    style={{ marginTop:8, display:"flex", alignItems:"center", gap:8,
                      background:"linear-gradient(90deg,#7c3aed,#6366f1)",
                      border:"none", color:"#fff", fontSize:13, padding:"10px 16px",
                      borderRadius:10, cursor:"pointer", width:"100%" }}>
                    <Download size={14}/>
                    <div style={{ textAlign:"left" }}>
                      <div style={{ fontWeight:500 }}>{msg.file.filename}</div>
                      <div style={{ fontSize:11, opacity:0.8 }}>
                        {msg.file.description} • {msg.file.rows_count} rows
                      </div>
                    </div>
                  </button>
                )}
        ...
        <div style={S.inputArea}>
          <div style={S.inputWrap}>
            <textarea
              ...
              placeholder={`Message ${dm.name}… (web search + file generation enabled)`}
              rows={1}
      ...
      <style>{`
        ...
        textarea:focus{border-color:rgba(167,139,250,0.5)!important;box-shadow:0 0 0 3px rgba(124,58,237,0.1)}
